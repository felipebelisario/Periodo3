...
int hamiltoniano(Grafo *gr,int *caminho){
    int i,j,aux,repetidos[gr->nro_vertices];
    
    for(i=0;i<gr->nro_vertices;i++){
        repetidos[i] = 0;               // Zera vetor de repetidos para testes no proximo for
    }

    for(i=0;i<gr->nro_vertices;i++){   // Verifica se possui vertices repetidos
        repetidos[caminho[i]]++;
        if(repetidos[caminho[i]] > 1){
            printf("\nPossui vertices repetidos! Nao eh hamiltoniano\n");
            return 0;
        }
    }

    for(j=0;j<gr->nro_vertices-1;j++){  // Verifica se a aresta testada é valida
        aux = 0;
        for(i=0;i<gr->grau[j]+1;i++){
            if(gr->arestas[caminho[j]][i] == caminho[j+1]){
                aux = 1;
            }
        }
        if(aux==0) {
    ...
    printf("\nEh hamiltoniano!\n");     // Caso não haja repetições e possuir arestas validas é hamiltoniano
    return 1;
}
...
// http://www.ime.usp.br/~pf/algoritmos_para_grafos/aulas/kruskal.html
void arvoreGeradoraMinimaKruskal_Grafo(Grafo *gr, int orig, int *pai){
    ...
    while(1){
        primeiro = 1;
        for(i=0; i < NV; i++){//percorre todos os v�rtices
            for(j=0; j<gr->grau[i]; j++){ // percorre as arestas do v�rtice
